window.onload = function() {
	...
	$(".character").on('click', function() {
	    ...
	    if (selectedHero === undefined) {
	        ...
	        $("#fightLog").html(""); //Text is cleared to look cleaner
	    }
	});
	...
	$(".character").on('mouseover', function() {
	    ...
	    //Setting variables to easily call and identify the correct ID tag in the HTML
	    var holder = $(this).attr("id");
	    var characterInfo = playableCharacters[holder];
	    //Displays character's name and his/her stats
	    $("#statsArea").html(characterInfo.name + " has " + characterInfo.hp + "HP and " + characterInfo.attack + " Singing Power");
	});

	//When user moves mouse away from the selectable characters area, the following messages are 
	//displayed to prevent the header from collapsing 
	...
	//Function to check the the HP stats of the hero and enemy after each fight/sing click event has occurred
	var whoIsDead = function() {
	    //Each time the fight click occurs, it checks the "wins" that the hero has incurred.
	    //If wins is less than 3 (there are only 3 characters to defeat), then this function checks whether
	    //hero or enemy is dead. 
	    if (wins < 3) {
	    		//If hero is dead, regardless of whether or not the enemy is dead, then you lose message displays
	        if (selectedHero.hp < 1) {
	            $("#instructions").html("<h2>You suck. You got served. Click reset to try again!</h2>");
	            $("#fightLog").html("Oh no! " + selectedHero.name + " has 0 HP left! Try again");
	        	//If enemy is dead and hero is alive, then you win that round, and select new challenger message displays     
	        } else if (selectedEnemy.hp < 1 && selectedHero.hp > 0) {
	            ...
	            $("#enemyArea").html(""); //Empties the enemy area image
	            $("#enemyAnnouncement").html(""); //Empties the enemy announcement area
	            selectedEnemy = undefined; //By setting this value to undefined, user can now select new enemy
	            wins++; //Increases wins
	        } 
	    }
	...
	function reset() {
	    ...
	    $("#fightLog").html("");
	    $("#instructions").html("");
	    //Removing classes that were added during character selection clicks
	    $(".character").removeClass("hidden");
	    $(".character").removeClass("selectedHero");
	    $(".character").removeClass("selectedEnemy");
	    //New announcement of choosing your hero
	    $("#statsArea").html("Choose your hero to begin");
